...
while [ ${ic} -le ${endvhr} ]; do
	...
	if [ -s ${conf_dir}/Ascii2Nc_hourly_obsAIRNOW.conf ]; then
	    ...
	    if [ ${SENDCOM} = "YES" ]; then
	        cp ${PREP_SAVE_DIR}/airnow_hourly_aqobs_${VDATE}${VHOUR}.nc ${COMOUTaqm}
	    fi
        else
...
if [ -s ${checkfile} ]; then
	...
	if [ ${SENDCOM} = "YES" ]; then
	    cp ${PREP_SAVE_DIR}/airnow_daily_${VDATE}.nc ${COMOUTaqm}
	fi
    else
...
then
	...
	if [ ${SENDCOM} = "YES" ]; then
            cat out1.grb2 out2.grb2 out3.grb2 > ${COMOUTaqm}/aqm.t${hour}z.max_8hr_o3${bctag}.${gridspec}.grib2
	fi
    else
        if [ ${SENDMAIL} = "YES" ]; then
