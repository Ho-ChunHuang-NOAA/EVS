...
## 
## Note the v7sp and v7 comout directory structure are different
## Check for correct scripts called
##
if [ "${fcst_input_ver}" == "v7sp" ]; then
    export dirname=aqm
    export gridspec=793
elif [ "${fcst_input_ver}" == "v7" -o "${fcst_input_ver}" == "v6" ]; then
    echo "EVS_CHECK :: This script exevs_aqmv7sp_stats.sh is not for ${fcst_input_ver}"
else
...
# check to see that model files exist, and list which forecast hours are to be used
    ...
    do
      filehr=$(printf %3.3d ${ihr})    ## fhr of grib2 filename is in 3 digit for aqmv7 and 3 digit for aqmv7sp
